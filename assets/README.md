...
interface delivers the admin macaroon in memory and the wrapper writes it with
mode `0600` only inside the ephemeral Nutshell subcontainer; the StartOS root CA
is prepared there the same way. Never add runes, macaroons, certificates, mint
seeds, databases, or copied dependency files to this directory.
