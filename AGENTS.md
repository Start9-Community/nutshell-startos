...
- **`startos/mintEnvironment.ts` stays SDK-free on purpose**, so `tests/mint-environment.test.ts` runs under plain `node --test` with no framework. Its sole runtime import is the explicit `.mjs` LND path contract, which plain Node and the package bundler resolve identically. Its config type is hand-written and its fields are **required** — that is what turns a rename in `fileModels/config.yaml.ts` into a compile error at the `main.ts` call site instead of a silently-defaulted variable.
...
- **The CLNRest rune is scraped from the interface's `?rune=` suffix.** That highly privileged credential is the only way a dependent reads it without mounting cln's volume, and it is a real contract with `cln-startos/startos/interfaces.ts`. A reactive watch deliberately retains the last rune during a temporary `null` suffix while CLN rotates it; a changed nonmatching suffix, a missing address, or a one-shot validation with no suffix still fails closed.
- **Fresh mints lock exactly one Lightning backend.** `store.json` may contain only `clnrest` or `lndrest`; absence creates the critical first-run task, invalid state fails closed, and the `0.20.3:2` migration assigns CLN to installations from `0.20.3:1` or earlier. Never add a reset, switch, automatic detection, or fallback path. Changing wallets underneath an established mint can break its accounting and strand outstanding ecash.
- **LND credentials come from its masked interface, not its volume.** Resolve the same-StartOS `control` bridge and `lnd-connect-rest` suffix reactively at runtime. Strictly decode the single canonical base64url `query.macaroon`, let the credential transit only the masked interface and wrapper memory, and write the raw bytes with mode `0600` to the fixed ephemeral Nutshell path. Do not persist it in wrapper state or intentionally place it in argv, environment values, action output, or logs. Neither backend receives a dependency volume mount; pinned StartOS does not support dependency file mounts end to end, and mounting all of `lnd/main` overexposes node state.
...
- **Backups preserve the locked backend.** `main` remains the required data volume, while the compatibility hooks copy `startos/store.json` when present so legacy one-volume backups still restore. Missing legacy wrapper state must migrate to CLN; it must not become a fresh choice. Restore tests belong on an isolated system, and the original and restored mint must never be exposed or run simultaneously.
